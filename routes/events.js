...
eventRoutes.delete('/:id/:photo', (req, res) => {
  ...
  cloudinary.v2.uploader.destroy(photo, function(error, result){
      Event.findByIdAndUpdate(id, {$pull: {photos: {public: photo}}}, {new: true}).then((event) => {
        res.redirect(`/events/${event.id}`);
